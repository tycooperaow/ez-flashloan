...
module.exports = async function(callback) {
  try {
    ...
    //let resultApprove = await kncContract.methods.approve(EXCHANGE_ADDRESS, TOKEN_ADDED).send({from: SETTINGS.from})
    console.log(`Successful approval txId: ${resultApprove.transactionHash}`)

    //add liquidity to exchange to be able to perform direkt token to token swap
    console.log('adding liquidity to exchange')
    let resultLiquidity = await exchangeContract.methods.addLiquidity(1, TOKEN_ADDED, DEADLINE).send({from: SETTINGS.from, value: 1000000000 })
